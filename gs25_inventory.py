...
def show_dashboard():
    ...
    if st.session_state.inventory_data.empty:
        ...
        with col1:
            if st.button("📁 파일 업로드하기", type="primary"):
                st.info("👈 사이드바에서 '📁 파일업로드'를 선택해주세요!")
        with col2:
            if st.button("🎯 데모 데이터 생성", type="secondary"):
                demo_data = pd.DataFrame({
                    '상품코드': ['8801234567890', '8801234567891', '8801234567892', '8801234567893'],
                    '상품명': ['삼각김밥 참치마요', '삼각김밥 불고기', '컵라면 신라면', '생수 500ml'],
                    '대분류': ['02', '02', '14', '11'],
                    '매가': [1200, 1300, 1800, 800],
                    '재고수량': [5, 8, 12, 25],
                    '추천재고수량': [15, 20, 30, 50],
                    '최종수정일': [datetime.now().strftime('%Y-%m-%d %H:%M:%S')] * 4
                })
